# ТЕХНИЧЕСКОЕ ЗАДАНИЕ: ФУНКЦИОНАЛЬНОСТЬ SENSIFY v1.10.0
...
✓ Статус: РЕАЛИЗОВАНО
  - Поле `expiresAt` в таблице shares; опция в Настройки → Аккаунт; `POST /api/share` принимает `expiresInDays`; `GET /api/share/[id]` отдаёт 404 после истечения
```
...
**Документ: функциональность Sensify v1.10.0**  
